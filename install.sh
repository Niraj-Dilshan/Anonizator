...
runin() {
	...
	{ "$@" 2>>../anonizator-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../anonizator-install.log
	done
}
...
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" anonizator-install.log
fi

if [ -d "Anonizator/hikka" ]; then
	cd Anonizator || {
...
if [ -f ".setup_complete" ]; then
	# If Anonizator is already installed by this script
	PYVER=""
...
echo "Installing..." >anonizator-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/s1zexxx/Anonizator/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Anonizator
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/s1zexxx/Anonizator || {
