...
class TokenObtainment(InlineUnit):
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("hikka.inline", "custom_bot", False):
                ...
                try:
                    ...
                    uid = utils.rand(6)
                    username = f"@anonizator_{uid}_bot"
            else:
                # Generate and set random username for bot
                uid = utils.rand(6)
                username = f"@anonizator_{uid}_bot"
            ...
            for msg in [
                f"🎈 Anonizator Userbot of {self._name}"[:64],
                username,
                "/setuserpic",
                username,
            ]:
    ...
    async def _assert_token(
        ...
        logger.info("Anonizator can't find token in database, taking it from BotFather")
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@anonizator_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
