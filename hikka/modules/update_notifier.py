...
class UpdateNotifierMod(loader.Module):
    """Tracks latest Anonizator releases, and notifies you, if update is required"""

    strings = {
        "name": "UpdateNotifier",
        "update_required": (
            "🎈 <b>Anonizator Update available!</b>\n\nNew GitHub commit pushed.\n💿"
            " <b>Anonizator <s>{}</s> -> {}</b>\n\nChanges: 👇🏻\n\n{}"
        ),
        "more": "\n<i><b>🎥 And {} more...</b></i>",
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "anonizator_update"},
                {"text": self.strings("ignore"), "data": "anonizator_upd_ignore"},
            ]
        )
        ...
        if self._pending not in [utils.get_git_hash(), self._notified]:
            m = await self.inline.bot.send_message(
                ...
                self.strings("update_required").format(
                    ...
                    '<a href="https://github.com/s1zexxx/Anonizator/compare/{}...{}">{}</a>'.format(
    ...
    async def update(self, call: InlineCall):
        ...
        if call.data not in {"anonizator_update", "anonizator_upd_ignore"}:
            return

        if call.data == "anonizator_upd_ignore":
